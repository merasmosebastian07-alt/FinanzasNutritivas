...
export function Services() {
  const { eyebrow, title, groups } = homeCopy.services;
          ...
          {groups.map((group) => (
            <div key={group.title}>
              ...
              {"intro" in group && group.intro ? (
                <p className="mt-3 max-w-3xl text-sm leading-relaxed text-paper sm:text-base">
                  {group.intro}
                </p>
              ) : null}
              <ol className="mt-4 divide-y divide-paper/10 border-y border-paper/10">
                {group.items.map((item, index) => (
                  <li
                    key={item.title}
                    className="grid gap-3 py-5 sm:grid-cols-12 sm:items-start sm:gap-8"
                  >
                    <p className="font-display text-sm text-gold-soft sm:col-span-2">
                      {String(index + 1).padStart(2, "0")}
                    </p>
                    <h4 className="font-display text-xl leading-snug text-paper sm:col-span-4 sm:text-2xl">
                      {item.title}
                    </h4>
